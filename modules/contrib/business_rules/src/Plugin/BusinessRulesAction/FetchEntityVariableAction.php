...
class FetchEntityVariableAction extends BusinessRulesActionPlugin {
  ...
  public function fetchEntityVariable($action, VariablesSet $event_variables) {
    ...
    $value = NULL;
    // PATCH to fetch by uuid - author : Jyothish Thomas
    $uuid_id=NULL;
    $tbundle='node';
    if ($id_field == 'uuid' && ($bundle=='infrastructure_group' || $bundle=='user')){
     if($bundle=='infrastructure_group'){
       $tbundle='group';
     }
     if($bundle=='user'){$tbundle='user';}
     $node_loaded_by_uuid = \Drupal::entityTypeManager()->getStorage($tbundle)->loadByProperties(['uuid' => $id]);
     //$node_loaded_by_uuid = reset($node_loaded_by_uuid);
     $uuid_id = array_keys($node_loaded_by_uuid)[0];
     switch ($tbundle) {
         case 'user':
            $id_field='uid';
            break;
